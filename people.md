---
layout: default
title: People
---

<div class="page-header">
    <div class="container">
        <h1 class="page-title">Our Team</h1>
        <p class="page-subtitle">Meet the researchers behind the CCG's FAST HI Surveys</p>
    </div>
</div>

<div class="container">
    <!-- Principal Investigators -->
    <section class="content-section">
        <h2 class="section-title">Principal Investigator</h2>
        
        <div class="people-grid">
            <div class="person-card">
                <img src="{{ '/assets/images/people/jie_wang.jpg' | relative_url }}" alt="Jie Wang" class="person-avatar">
                <h3 class="person-name">Prof. Jie Wang (王杰)</h3>
                <p class="person-role">Principal Investigator; Group Leader</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> Numerical simulations, galaxy formation and evolution, Milky Way, large-scale structures, radio astronomy, deep learning.</p>
                    <p><strong>Webpage:</strong> <a href="https://nao.cas.cn/jypy/ds/bsds/202204/t20220414_6986175.html" target="_blank">Link</a></p>
                    <p><strong>Email:</strong> jie.wang[at]nao.cas.cn</p>
                </div>
            </div>
        </div>
    </section>
    
    <!-- Researchers -->
    <section class="content-section">
        <h2 class="section-title">Core Researchers</h2>
        
        <div class="people-grid">
            <div class="person-card">
                <img src="{{ '/assets/images/people/yingjie_jing.jpg' | relative_url }}" alt="Yingjie Jing" class="person-avatar">
                <h3 class="person-name">Dr. Yingjie Jing (景英杰)</h3>
                <p class="person-role">Postdoctoral Researcher</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> Galaxy formation, HI gas, numerical simulations, machine learning.</p>
                    <p><strong>Email:</strong> jyj[at]nao.cas.cn</p>
                </div>
            </div>

            <div class="person-card">
                <img src="{{ '/assets/images/people/Tiantian_Liang_梁甜甜.jpg' | relative_url }}" alt="Tiantian Liang" class="person-avatar">
                <h3 class="person-name">Dr. Tiantian Liang (梁甜甜)</h3>
                <p class="person-role">Lecturer</p>
                <p class="person-affiliation">North China Electric Power University</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> Radio astronomy, HI observations, interactions between SNR and the ISM.</p>
                    <p><strong>Email:</strong> 51752684[at]ncepu.edu.cn</p>
                </div>
            </div>
        
        
            
            <div class="person-card">
                <img src="{{ '/assets/images/people/Chen_Xu_徐晨_.jpg' | relative_url }}" alt="Chen Xu" class="person-avatar">
                <h3 class="person-name">Chen Xu (徐晨)</h3>
                <p class="person-role">PhD Student</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> Radio astronomy, HI gas in galaxies, galaxy formation.</p>
                    <p><strong>Webpage:</strong> <a href="https://stellarxu.github.io/" target="_blank">Link</a></p>
                    <p><strong>Email:</strong> xuchen[at]nao.cas.cn</p>
                </div>
            </div>

            <div class="person-card">
                <img src="{{ '/assets/images/people/Qingze_Chen_陈箐泽_.jpg' | relative_url }}" alt="Qingze Chen" class="person-avatar">
                <h3 class="person-name">Qingze Chen (陈箐泽)</h3>
                <p class="person-role">PhD Student</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> HI observation of galaxies, dark galaxies.</p>
                    <p><strong>Webpage:</strong> <a href="https://qingzechen.github.io/" target="_blank">Link</a></p>
                    <p><strong>Email:</strong> chenqz[at]bao.ac.cn</p>
                </div>
            </div>

            <div class="person-card">
                <img src="{{ '/assets/images/people/Zerui_Liu_刘泽瑞_.jpg' | relative_url }}" alt="Zerui Liu" class="person-avatar">
                <h3 class="person-name">Zerui Liu (刘泽瑞)</h3>
                <p class="person-role">PhD Student</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> HI 21cm cosmology, galaxy formation, large scale structure formation.</p>
                    <p><strong>Email:</strong> liuzr[at]bao.ac.cn</p>
                </div>
            </div>

            <div class="person-card">
                <img src="{{ '/assets/images/people/Zhipeng_Hou_侯志鹏_.jpg' | relative_url }}" alt="Zhipeng Hou" class="person-avatar">
                <h3 class="person-name">Zhipeng Hou (侯志鹏)</h3>
                <p class="person-role">PhD Student</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> Galaxy formation and evolution, HI observation.</p>
                    <p><strong>Email:</strong> houzp[at]bao.ac.cn</p>
                </div>
            </div>

            <div class="person-card">
                <img src="{{ '/assets/images/people/Yiwei_Xu_徐艺玮_.jpg' | relative_url }}" alt="Yiwei Xu" class="person-avatar">
                <h3 class="person-name">Yiwei Xu (徐艺玮)</h3>
                <p class="person-role">Master Student</p>
                <p class="person-affiliation">NAOC, CCG</p>
                <div class="person-detail">
                    <p><strong>Research:</strong> Galaxy formation, HI.</p>
                    <p><strong>Email:</strong> xuyw[at]bao.ac.cn</p>
                </div>
            </div>



        </div>
    </section>
    
    <!-- Collaborators -->
    <section class="content-section">
        <h2 class="section-title">Collaborators</h2>
        
        <p>The HD² survey is a collective effort involving many collaborators. Below is a list of researchers who have contributed to the project (excluding the core team). <strong>Note that this list is a partial list of our collaborators in no particular order.</strong> ★ denotes student members.</p>

        <table>
            <thead>
                <tr>
                    <th>Name</th>
                    <th>Affiliation</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>Jing Wang</td>
                    <td>Kavli Institute for Astronomy and Astrophysics, PKU</td>
                </tr>
                <tr>
                    <td>Hu Zou</td>
                    <td>National Astronomical Observatories, CAS</td>
                </tr>
                <tr>
                    <td>Wei Du</td>
                    <td>National Astronomical Observatories, CAS</td>
                </tr>
                <tr>
                    <td>Hong Guo</td>
                    <td>Shanghai Astronomical Observatory, CAS</td>
                </tr>
                <tr>
                    <td>Liang Gao</td>
                    <td>Beijing Normal University</td>
                </tr>
                <tr>
                    <td>Qi Guo</td>
                    <td>Beijing Normal University</td>
                </tr>
                <tr>
                    <td>Qiusheng Gu</td>
                    <td>Nanjing University</td>
                </tr>
                <tr>
                    <td>Zhen Jiang</td>
                    <td>National Astronomical Observatories, CAS</td>
                </tr>
                <tr>
                    <td>Xi Kang</td>
                    <td>Zhejiang University</td>
                </tr>
                <tr>
                    <td>Xu Kong</td>
                    <td>University of Science and Technology of China</td>
                </tr>
                <tr>
                    <td>Cheng Li</td>
                    <td>Tsinghua University</td>
                </tr>
                <tr>
                    <td>Jun Pan</td>
                    <td>National Astronomical Observatories, CAS</td>
                </tr>
                <tr>
                    <td>Tao Wang</td>
                    <td>Nanjing University</td>
                </tr>
                <tr>
                    <td>Wenting Wang</td>
                    <td>Shanghai Jiao Tong University</td>
                </tr>
                <tr>
                    <td>Taotao Fang</td>
                    <td>Xiamen University</td>
                </tr>
                <tr>
                    <td>Niankun Yu</td>
                    <td>Max Planck Institute for Radio Astronomy</td>
                </tr>
                <tr>
                    <td>Xiao Li</td>
                    <td>Max Planck Institute for Radio Astronomy</td>
                </tr>
                <tr>
                    <td>Huijie Hu</td>
                    <td>Anqing Normal University</td>
                </tr>
                <tr>
                    <td>Ziming Liu </td>
                    <td>National Astronomical Observatories, CAS</td>
                </tr>
                <tr>
                    <td>Pinsong Zhao</td>
                    <td>Kavli Institute for Astronomy and Astrophysics, PKU</td>
                </tr>
                <tr>
                    <td>Yanrui Zhou ★</td>
                    <td>Shanghai Jiao Tong University</td>
                </tr>

            </tbody>
        </table>

        <div class="info-card" style="background-color: #f8f9fa; border-left: 5px solid #007bff; padding: 1.5rem; margin-top: 2rem; border-radius: 4px;">
            <p style="margin-bottom: 0;"><strong>Join Us:</strong> If you are interested in joining the collaboration, please contact the <a href="mailto:">core team</a>. We welcome researchers and students who are interested in our FAST HI surveys.</p>
        </div>
    </section>
    
    
</div>
